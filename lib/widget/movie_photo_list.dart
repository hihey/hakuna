...
class MoviePhotoList extends StatefulWidget {
  ...
  const MoviePhotoList({Key key, this.action, this.title, this.id}) : super(key: key);
...
class _MoviePhotoListState extends State<MoviePhotoList> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {// 滚动监听注册
  ...
  Future<void> fetchData() async {
    ...
    switch (action) {
      case 'actor':
        data = await client.getActorPhotos(actorId: this.widget.id, start: start, count: count);
        break;
      case 'movie':
        data = await client.getMovieAlbum(movieId: this.widget.id, start: start, count: count);
        break;
    }
