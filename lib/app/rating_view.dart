...
class StaticRatingBar extends StatelessWidget {
  ...
  final int count;/// number of stars
  final double rate;/// init rate
  final double size;/// size of the starts
...
class _PainterStars extends CustomPainter {
  ...
  Path createStarPath(double radius, Path path) {
    ...
    double radiusIn = (radius * Math.sin(radian / 2) / Math.cos(radian)) * 1.1; // 中间五边形的半径,太正不是很好看，扩大一点点
